...
| **Anhänge** + Vorschaubild | Child-IOs unter der Idee (`ccm:childio`, Aspekt `ccm:io_childobject`) bzw. Node-Preview |
...
- **Service-Account** (`EDU_GUEST_*` aus `.env`): schreibt nur im Namen Anonymer
  in die Inbox (s. Inbox-Pattern).
...
2. **Einsortieren** (Moderation) → die Idee wird per **`addReference`** in eine
   Herausforderungs-Sammlung verlinkt — **nicht** `_move`. Das Original bleibt in
   der Inbox, die Sammlung bekommt einen `ccm:io_reference` mit `originalId`.
...
> Ziel-Sammlung anlegen (idempotent gegen 409), alte Reference löschen. Die App
> führt pro Idee **genau eine** Topic-Referenz.
...
2. **Periodischer Voll-Sync (eventual, alle 5 min):** fängt Änderungen ab, die
...
| die edu-sharing-Oberfläche direkt | nach dem **nächsten Sync** (≤ 5 min) |
...
Browser ──POST /ideas (+ Captcha)──► Backend
   ...
   (Inbox-Knoten: NICHT im Public-Cache)
        … später, Moderation:
   Mod ──POST /moderation/move──► Backend ──addReference──► edu-sharing (Challenge)
