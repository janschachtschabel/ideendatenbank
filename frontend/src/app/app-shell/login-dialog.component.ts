...
const REGISTER_URL = 'https://wirlernenonline.de/register/';
...
@Component({
  ...
  styles: [`
    ...
    .box { background: #fff; border-radius: 12px; padding: 24px 28px 22px;
    ...
    label { display: block; font-size: .82rem; font-weight: 600; color: var(--wlo-text);
            margin: 2px 0 4px; }
    input { width: 100%; padding: 9px 12px; border: 1px solid var(--wlo-border);
    ...
    button.btn.primary { background: var(--wlo-primary); color: #fff;
      &:hover:not(:disabled) { background: var(--wlo-primary-600); }
      &:disabled { opacity: .5; cursor: not-allowed; } }
    button.btn.secondary { background: var(--wlo-bg); color: var(--wlo-text);
      &:hover { background: #e6edf7; } }
    .note { font-size: .78rem; color: var(--wlo-muted); margin-top: 10px; line-height: 1.45; }
    .error { background: #fff0f0; border: 1px solid #e1a5ac; color: #b00020;
             padding: 8px 12px; border-radius: 6px; font-size: .85rem; margin-bottom: 10px; }

    /* Register-CTA */
    .register-card {
      background: linear-gradient(135deg,
        color-mix(in srgb, var(--wlo-primary) 8%, #fff),
        color-mix(in srgb, var(--wlo-primary) 3%, #fff));
    ...
    .register-card h3 {
      margin: 0 0 10px; font-size: 1.05rem; color: var(--wlo-primary);
      display: flex; align-items: center; justify-content: center; gap: 8px;
    }
    .register-card p { margin: 0 0 14px; font-size: .92rem; color: var(--wlo-text); line-height: 1.55; }
    .register-card .cta {
      display: inline-flex; align-items: center; gap: 8px;
      background: var(--wlo-primary); color: #fff; text-decoration: none;
      padding: 11px 22px; border-radius: 8px; font-weight: 600;
      transition: background .15s;
      &:hover { background: var(--wlo-primary-600); }
