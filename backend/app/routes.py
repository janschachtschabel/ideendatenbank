...
async def get_idea(idea_id: str, authorization: str | None = Header(None)):
    ...
    # Die zwei langsamen Live-Reads (Node-Metadaten + Kommentare) PARALLEL holen
    # statt seriell — auf dem langsamen Alfresco-Backend ist das der Hauptanteil
    # der Detail-Latenz. node_metadata liefert Rating + Owner UND (bei io-Ideen)
...
# Die Auth-Prädikate (is_moderator/verify_login/is_owner_or_mod/can_edit_idea +
# decode_basic_user) liegen jetzt in app/auth.py und werden oben unter ihren
# bisherigen Namen re-importiert. `_require_moderator` bleibt hier, weil es HTTP
# (401/403) und Audit-Logging (_log_activity) bündelt — beides Route-Belange.
